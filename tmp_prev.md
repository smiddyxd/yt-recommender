Ultimate Project Overview â€” YT Manager
...
- How to update (checklist):
  1) Bump â€œVerified As Ofâ€ date below. 2) Add/remove items in Architecture and Storage Model. 3) Reflect new/changed messages under Messaging Protocol. 4) Note any new user-visible flows in UI. 5) Record notable behavior changes in Changelog.
...
- Build: `npm run build` â†’ `dist/`; load unpacked extension from `dist` in Chromium-based browser.
- Dev watch (run each in separate terminals): `npm run watch:bg`, `watch:cs`, `watch:opt`, `watch:pop`.
- First-use: Open Options, click â€œRefresh dataâ€ and provide your YouTube API key (stored in `chrome.storage.local.ytApiKey`).
...
- Shared/Types
  - `src/shared/conditions.ts`: Condition AST, evaluation for videos/channels; â€œGroupâ€ type (called â€œPresetâ€ in UI).
...
5) On mutations, background records lightweight events â†’ commits; appends to `events-YYYY-MM.jsonl` in Drive and occasionally saves snapshots.

Auto-Scrape & Presets
- Auto-scrape runs every ~2s only if user was active within the last 10s. Disabled on channel pages and all playlist pages.
- Current watch page is always captured; other tiles are captured only if accepted by at least one enabled â€œPresetâ€.
...
- Stores and key fields
  - `videos` (keyPath: `id`) â€” indexes: `byChannel` on `channelId`, `byTag` on `tags` (multiEntry).
  - `trash` (keyPath: `id`) â€” index: `byDeletedAt`.
  - `tags` (keyPath: `name`) â€” index: `byCreatedAt`; record: `{ name, color?, createdAt?, groupId? }`.
  - `tag_groups` (keyPath: `id`) â€” indexes: `byName`, `byCreatedAt`.
  - `groups` (keyPath: `id`) â€” indexes: `byName`, `byUpdatedAt`; record includes `scrape?: boolean`.
  - `channels` (keyPath: `id`) â€” indexes: `byName`, `byFetchedAt`.
  - `channels_trash` (keyPath: `id`) â€” index: `byDeletedAt`.
  - `channels_pending` (keyPath: `key`) â€” index: `byCreatedAt`; rows like `{ key: 'handle:@foo' | 'name:Some Name', name?, handle?, createdAt?, updatedAt? }`.
  - `meta` (keyPath: `key`) â€” holds aggregated lists like `{ key: 'videoTopics', list: string[] }`.
  - `events_commits` (keyPath: `commitId`) â€” index: `byTs`.
  - `events` (keyPath: `id`) â€” index: `byCommit`.
...
- Files written:
  ...
  - Optional `cutoff.json` or `cutoff-*.json` marker after â€œDelete up to hereâ€.
...
- Backlog replay: if Drive append fails, commit ids queue in `chrome.storage.local['drive.unsyncedCommitIds']`; background periodically replays to Drive. Options header shows a â€œDrive backlog: Nâ€ badge when > 0.
...
- Filters editor: AND/OR/NOT linear editor for many predicates (video/channel/tag/source/topic); Save as â€œPresetâ€ (backed by Group). Not all predicates are scrape-checkable â€” UI disables scrape toggle when unsupported.
...
- Refresh panel: stubs count, â€œRefresh dataâ€ (videos), â€œFetch channels (unfetched)â€.
...
Popup Highlights
- Shows current page context (watch/channel/other); â€œScrape current pageâ€; toggle â€œAuto-capture stubs on watch pagesâ€.
...
- â€œGroupâ€ in storage/type is labeled â€œPresetâ€ in UI; keep the permanent clarification comments in code and this doc.
...
- New predicates: update `src/shared/conditions.ts` (matchers) and extend Filters UI (`src/ui/options/lib/filters.ts`, `FiltersBar.tsx`). If you want auto-scrape to evaluate them, add support in content scrape-time evaluator and list predicate under â€œscrape-checkableâ€.
...
Changelog (concise)
- 2025-09-01: Restore & Apply (dry run/apply) added; pending channels pipeline refined (gated, de-duped, batch resolver); Drive auth made silent by default; backlog replay and â€œDrive backlogâ€ badge added; history routes and UI extended; selective API-change events logged on refresh.

Notes / TODO
- `rules` store and `rules/*` message types are presently stubs; no background routes implemented yet.
- Consider optional â€œvisible folderâ€ backup mode (Drive `drive.file`) if needed in future; current implementation targets appData only.
...
- Dev watch (run in parallel terminals):
  - `npm run watch:bg` â€” background service worker bundle to `dist/background/`
  - `npm run watch:cs` â€” content script bundle to `dist/content/`
  - `npm run watch:opt` â€” Options UI to `dist/ui/options/`
  - `npm run watch:pop` â€” Popup UI to `dist/ui/popup/`
- Load in Chrome/Chromium: Extensions â†’ Developer Mode â†’ Load unpacked â†’ select `dist/` (manifest is `dist/manifest.json`). Reload extension after background changes.
...
- Keep push notifications (`db/change { entity }`) accurate so UIs refresh only whatâ€™s needed.
- Maintain the â€œPreset vs Groupâ€ naming: storage/type is Group; UI labels as Preset. Keep the permanent comments in code and this overview.
...
- Document new routes under â€œMessaging Protocolâ€.

Predicates & Filters (endâ€‘toâ€‘end)
...
- Auto-scrape support: if the predicate should be checkable at scrape-time, add handling to contentâ€™s evaluator and keep the â€œcheckableâ€ list in sync:
...
- Update this docâ€™s â€œAutoâ€‘Scrape & Presetsâ€ section with any changes to checkable predicates.

Autoâ€‘Scrape & Sources Field
...
- Lazy debug loads: â€œShow infoâ€ toggles fetch full rows for display to reduce baseline payload.
...
Logging & Debug
- Debug helpers in `src/types/debug.ts` â€” set `DEBUG=false` to silence logs.
...
- Backup: â€œVersion Historyâ€ shows commits; download commit works; usage numbers present.
- History cutoffs: â€œDelete up to hereâ€ creates cutoff marker; â€œDownload up to hereâ€ bundles lines; import verifies marker.
...
- Add a new tag predicate for videos:
  ...
  3) If needed at scrape-time, support in content evaluator and UIâ€™s `isPresetScrapeCheckable`
  4) Update this docâ€™s predicate lists
...
Housekeeping
- Keep this fileâ€™s â€œVerified As Ofâ€ date current when semantics change.
