...
- Background
  ...
  - `src/background/driveBackup.ts`: Google Drive appData auth + read/write (JSON, JSONL, snapshots). Plaintext storage only (no encryption). Silent auth by default; pass `{ interactive: true }` when prompting is desired.
...
- Files written:
  - `settings.json` (latest snapshot; plaintext only; encryption removed).
  - `snapshots/settings-YYYYMMDD-HHMMSS.json` (dynamic checkpoints). Baseline bootstrap: background creates an initial snapshot if none exists (on startup silently; and after setting Drive Client ID interactively).
...
- Export slicing: “Download up to here” and “Delete up to here” operate on whole commits (include/remove all lines sharing the `commitId`) to avoid slicing away fast-follow events.
- Unicode-safe downloads: commit JSONL and “up to” bundles are returned as UTF‑8 base64 content; UI decodes to bytes before Blob construction.
- New routes:
  - `backup/history/revertTo { commitId, dryRun? }` — revert to snapshot at/near the target and replay forward.
  - `backup/history/snapshotNow { interactive?, name? }` — create a named snapshot; also triggers backlog replay.
- Manual “Backup settings” flow: finalizes any pending commit (`finalizeCommitAndFlushIfAny`), saves `settings.json` (interactive if needed), then triggers backlog replay (`replayUnsyncedCommitsToDrive`).
...
- Apply (`backup/restore/apply`): merge/overwrite semantics; selective application of `channelTags`, `videoTags`, `sources`, `progress`. In merge mode, tag-name dedupe (case/trim) and tag-group remap by name avoid duplicates; incoming references map to local canonical names/groups.
