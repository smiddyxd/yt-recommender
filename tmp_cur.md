...
- Build: npm run build â†’ dist/; load unpacked from dist in a Chromium-based browser.
- Dev watch (run each in separate terminals): watch:bg, watch:cs, watch:opt, watch:pop.
- First-use: open Options, click â€œFetch video dataâ€, provide your YouTube API key (stored in chrome.storage.local.ytApiKey).
...
- Shared/Types
  - src/shared/conditions.ts: Condition AST and evaluators for videos/channels (â€œGroupâ€ in storage; labeled â€œPresetâ€ in UI).
...
5) Mutations record lightweight events â†’ commits; appended to events-YYYY-MM.jsonl in Drive; periodic snapshots.
...
- Stores
  ...
  - tags (keyPath: name) â€” index: byCreatedAt; row: { name, color?, createdAt?, groupId? }.
  - tag_groups (keyPath: id) â€” indexes: byName, byCreatedAt.
  - groups (keyPath: id) â€” indexes: byName, byUpdatedAt; row includes scrape?: boolean.
...
- Content â†’ Background: cache/VIDEO_SEEN, cache/VIDEO_STUB, cache/VIDEO_PROGRESS, cache/VIDEO_PROGRESS_PCT
...
- Dynamic checkpoints: when commit processing weight â‰¥ 10k or month size â‰¥ 20 MB; daily alarm also saves settings.
- Backlog replay: if Drive append fails, commit IDs queue in chrome.storage.local['drive.unsyncedCommitIds'] and later replay silently. Manual â€œBackup settingsâ€ finalizes any pending commit and also triggers backlog replay.
- Downloads are UTFâ€‘8 safe (commit JSONL and â€œup toâ€ bundles).
...
- Filters editor: AND/OR/NOT linear editor for many predicates; Save as â€œPresetâ€ (backed by Group). Not all predicates are scrape-checkable.
- Tags sidebar: Tag CRUD, Tag Groups CRUD, assign tags to groups; pickers grouped by Tag Group.
- Top bar: single toggles for List/Grid, Videos/Channels, and Trash; â€œRefresh DBâ€ (reload local list), â€œFetch video dataâ€ (videos via API), â€œFetch unfetched channelsâ€.
- Stubs indicator: checkbox label shows â€œX stubsâ€ (total across videos+channels) with a second line â€œY in viewâ€.
...
Popup Highlights
- Shows current page context (watch/channel/other); â€œScrape current pageâ€; toggle â€œAutoâ€‘capture stubs on watch pagesâ€.
- Tag current video/channel using grouped tag pickers; channel auto-tag helper applies a â€œ.taggedâ€ tag alongside the chosen tag.
...
Changelog (concise)
- 2025-09-05
  - Drive backups: encryption/passphrase removed â€” all snapshots/history stored as plaintext JSON/JSONL. Silent auth by default; â€œBackup settingsâ€ finalizes any pending commit and replays backlog.
  - History downloads: commit and â€œup toâ€ bundles use UTFâ€‘8 encoding (Unicodeâ€‘safe).
  - Options Top Bar: single toggles for List/Grid, Videos/Channels, Trash; removed â€œChannel trashâ€ and â€œSelect pageâ€; â€œRefreshâ€ renamed to â€œRefresh DBâ€.
  - Stubs indicator: merged into checkbox label showing total across videos+channels and a second line â€œX in viewâ€.
- 2025-09-01
