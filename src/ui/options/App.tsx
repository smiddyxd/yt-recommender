...
// ---- IndexedDB helpers (read-only here) ----// Project rows to a slim shape (drop heavy fields like raw `yt` payload)
...
function App() {
    ...
    const videoSourcesOptionsMemo = useMemo((): Array<{
        ...
        let base = videos;
        if (cond) {
            base = base.filter(v => matches(v as any, cond, { resolveGroup: (id) => groups.find(g => g.id === id), resolveChannel: (id) => channels.find(c => c.id === id) as any }));
    ...
    async function loadChannelsDir() {
    ...
    } useEffect(() => {    // initial load also pulls groups    refresh();
        loadGroups();
        loadChannelsDir();
        loadTopicOptions();
        // Drive client id    
        ...
        } catch { }    // load last refresh/backup time from storage    
        try {
...
async function loadRecSets() {
    ...
    async function purgeSelected() {
        ...
        // Undo only applies to videos (channels use explicit Restore in trash view)    await sendBg('videos/restore', { ids: lastDeleted });
        setShowUndo(false);
        setLastDeleted(null);
        await refresh();
    ...
    } function selectAllMatching(allIds: string[]) {    // Union with existing selection; preserve disabled(hidden) selections
        setSelected(prev => {
            const next = new Set(prev);
    ...
    async function loadCollections() {
        ...
        setChain([]);
        refresh();
        // reload from the correct store (videos vs trash)    
        loadTags();
        loadTagGroups();
        loadCollections();
    ...
    // (moved) tagCounts is computed after visible/hidden selection is derived  // AFTER: derive names from the registry we loaded via tags/list  // All registry tags (for the tag apply UI)  
    ...
    function toggleTag(tag: string) {
        if (inChannels) {
            ...
            sendBg('channels/applyTags', { ids: Array.from(selected), addIds: allHaveCh ? [] : [tag], removeIds: allHaveCh ? [tag] : [] }).then(() => loadChannelsDir());
            return;
        } const count = tagCounts.get(tag) || 0;
        const allHave = count === selectedCount && selectedCount > 0;
        // If all have it â†’ remove from all; otherwise add to all    
        sendBg('videos/applyTags', { ids: Array.from(selected), addIds: allHave ? [] : [tag], removeIds: allHave ? [tag] : [] }).then(() => refresh());
        ...
        } catch { } const needle = q.trim().toLowerCase();
    ...
    const channelsFiltered = useMemo(() => {  // Apply boolean filter condition first (channel + video cross-scope), then search filter by text  
        const cond = chainToCondition(chain);
        let base = channels;
        if (showStubsOnly) base = base.filter(ch => !Number.isFinite((ch.fetchedAt as any) || undefined));
        if (cond) {
            base = channels.filter(ch => matchesChannel(ch as any, cond as any, { videos, resolveGroup: (id) => groups.find(g => g.id === id) }));
    ...
    const selectedVisibleSetNormal = useMemo(() => {
        ...
        selected.forEach(id => {
            if (visibleIdsNormal.has(id)) s.add(id);
        });
        return s;
    ...
    const displayVideos = useMemo(() => {
        if (showDisabledOnly) {      // show selected items that are hidden by current filters      return videos.filter(v => selected.has(v.id) && !visibleIdsNormal.has(v.id));
        } return filtered;
    ...
    const videoTagOptions = useMemo((): Array<{
        name: string;
        count: number
        ...
        } catch { } const numCmp = (a: string, b: string) => {
            const ai = /^\d+$/.test(String(a)) ? parseInt(String(a), 10) : NaN;
            const bi = /^\d+$/.test(String(b)) ? parseInt(String(b), 10) : NaN;
            const aNum = Number.isFinite(ai), bNum = Number.isFinite(bi);
            if (aNum && bNum) return ai - bi;
        ...
        return Array.from(counts, ([name, count]) => ({ name, count })).sort((a, b) => numCmp(a.name, b.name));
    ...
    const channelTagOptions = useMemo((): Array<{
        ...
        const pruned = chain.filter(e => !(e.pred.kind === 'c_tags_any' || e.pred.kind === 'c_tags_all' || e.pred.kind === 'c_tags_none' || e.pred.kind === 'v_tags_any' || e.pred.kind === 'v_tags_all' || e.pred.kind === 'v_tags_none'));
        const cond = chainToCondition(pruned);
        let base = channels;
        if (cond) {
            base = channels.filter(ch => matchesChannel(ch as any, cond as any, { videos, resolveGroup: (id) => groups.find(g => g.id === id) }));
    ...
    function applySort<A extends any>(arr: A[], fields: Array<{
        field: string;
        dir: 'asc' | 'desc'
    }>, kind: 'videos' | 'channels'): A[] {
        ...
        if (!videoSorts.length) {      // default for videos      
    ...
    async function ensureApiKey(): Promise<string | null> {
        ...
        try {
            ...
            // flip to false to refetch everything      
            await sendBg('videos/refreshAll', { skipFetched: SKIP_FETCHED });
            const now = Date.now();
            setLastRefreshAt(now);
            try {
                chrome.storage?.local?.set({ lastRefreshAt: now });
            } catch { } await refresh();
        } catch (e: any) {
        ...
        try {
            const [v, c] = await Promise.all([sendBg('videos/stubsCount', {} as any).catch(() => ({ ok: false, count: 0 })), sendBg('channels/stubsCount', {} as any).catch(() => ({ ok: false, count: 0 })),]);
    ...
    function normTag(s: string): string {
        ...
        for (const t of allTags) {
            try {
                await sendBg('tags/create', { name: t });
        ...
        try {
            ...
            // message-size safety; BG chunks to 50 for API      
            for (const c of chunks) {
                await sendBg('channels/refreshByIds', { ids: c });
            }
        } catch { /* non-fatal */ }    // Refresh UI lists    
        ...
        refreshStubCount();
        const h = (msg: any) => {
            if (msg?.type === 'db/change' && msg?.payload?.entity === 'videos') refreshStubCount();
        };
    ...
    useEffect(() => {
        if (mode === 'subs') void maybeLoadMoreSubsWindow();
    ...
    function applyTagToSelection(tag: string) {
        if (inChannels) {
            ...
            sendBg('channels/applyTags', { ids: selectedIds, addIds: haveAll ? [] : [tag], removeIds: haveAll ? [tag] : [] }).then(() => loadChannelsDir());
            return;
        ...
        sendBg('videos/applyTags', { ids: Array.from(selectedVisibleSetDisplay), addIds: haveAllVideos ? [] : [tag], removeIds: haveAllVideos ? [tag] : [] }).then(() => refresh());
    ...
    const subsAll = useMemo(() => {    // Base candidates before user filters; chronological sort by uploadedAt desc
    ...
    const subsFiltered = useMemo(() => {    // Apply FiltersBar condition and hide-by-default behavior like manager    
        const cond = chainToCondition(chain);
        let base = subsAll;
        if (cond) {
            base = base.filter(v => matches(v as any, cond, { resolveGroup: (id) => groups.find(g => g.id === id), resolveChannel: (id) => channels.find(c => c.id === id) as any }));
    ...
    const subsSelectedVisibleSetNormal = useMemo(() => {
        const s = new Set<string>();
        selected.forEach(id => {
            if (subsVisibleIdsNormal.has(id)) s.add(id);
        });
        return s;
    ...
    const subsSelectedHiddenCount = useMemo(() => {
        let n = 0;
        selected.forEach(id => {
            if (!subsVisibleIdsNormal.has(id)) n++;
        });
        return n;
    }, [selected, subsVisibleIdsNormal]);
    ...
    function applyTagToSubsSelection(tag: string) {
        ...
        sendBg('videos/applyTags', { ids: Array.from(subsSelectedVisibleSetDisplay), addIds: haveAll ? [] : [tag], removeIds: haveAll ? [tag] : [] }).then(() => refresh());
    } function openSubsInTabs() {
        const ids = Array.from(subsSelectedVisibleSetDisplay);
        if (!ids.length) return;
        for (const id of ids) {
            try {
                chrome.tabs?.create?.({ url: `https://www.youtube.com/watch?v=${id}`, active: false });
            } catch { }
        }
    ...
    async function loadSubsInitialWindow() {
        ...
        try {
            ...
            if (combined.length > keep) {
                ...
                if (nextPage > 1) nextPage = nextPage - 1;
    ...
    } useEffect(() => {    // Prefer URL hash deep-link; fallback to storage
    ...
    useEffect(() => {    // Persist to storage and reflect in hash    
        try {
            chrome.storage?.local?.set({ 'options.mode': mode });
        } catch { } setHashParams({ mode });
    }, [mode]);
    useEffect(() => {
        try {
            chrome.storage?.local?.set({ 'options.view': view });
    ...
    return (<div className="page" data-mode={mode}><Sidebar tags={tags} newTag={newSidebarTag} setNewTag={setNewSidebarTag} tagEditing={tagEditing} tagEditValue={tagEditValue} setTagEditValue={setTagEditValue} startRename={startRename} cancelRename={cancelRename} commitRename={commitRename} addTag={addTag} removeTag={removeTag} importing={importing} importMessage={importMessage} onImportFile={handleImportFile} tagGroups={tagGroups} onCreateTagGroup={createTagGroup} onRenameTagGroup={renameTagGroup} onDeleteTagGroup={deleteTagGroup} onUpdateTagGroup={updateTagGroup} onAssignTagToGroup={assignTagToGroup} groups={groups} startEditFromGroup={startEditFromGroup} removeGroup={removeGroup} isPresetScrapeCheckable={isPresetScrapeCheckable} toggleGroupScrape={toggleGroupScrape} driveClientId={driveClientId} onSetDriveClientId={setDriveClientIdInteractive} onBackupNow={backupSettingsInteractive} respectDontRecommend={respectDontRecommend} collections={collections} activeCollectionId={activeCollectionId} onOpenCollection={openCollection} mode={mode} onModeChange={(m) => setMode(m)} />      <div className="content">        {/* Non-manager content (Subs / Recommender) */}        <div className="non-manager-only">          <header>            <div className="controls">              {/* View toggle (single button) */}              <div className="view-toggle" role="group" aria-label="View mode">                <button type="button" className="icon-btn" aria-pressed={true} title={isList ? 'Switch to grid view' : 'Switch to list view'} onClick={() => setLayout(isList ? 'grid' : 'list')}                >                  {isList ? (<svg className="icon" viewBox="0 0 24 24" aria-hidden="true">                      <rect x="5" y="5" width="14" height="14" rx="2" ry="2"></rect>                    </svg>) : (<svg className="icon" viewBox="0 0 24 24" aria-hidden="true">                      <path d="M4 7h16v2H4zM4 11h16v2H4zM4 15h16v2H4z"></path>                    </svg>)}                </button>              </div>              {/* Selection controls (videos only) */}              {mode !== 'recommender' && (<div className="sel-controls">                <button type="button" className="btn-ghost" title="Select all (matching filter)" onClick={() => setSelected(new Set(subsDisplayVideos.map(v => v.id)))}>all</button>                <button type="button" className="btn-ghost" title="Clear selection" onClick={clearSelection} disabled={selected.size === 0}>C</button>                <button type="button" className="btn-ghost" title="Invert selection (within current filter)" onClick={() => {
    ...
    }} disabled={subsDisplayVideos.length === 0}>Inv</button>                <button type="button" className="btn-ghost" title="Open selected in new tabs" onClick={openSubsInTabs} disabled={subsSelectedVisibleSetDisplay.size === 0}>T</button>                <button type="button" className="btn-ghost" title={showDisabledOnly ? 'Show filtered results' : 'Show disabled selection'} onClick={() => setShowDisabledOnly(v => !v)}>D</button>                {/* Hide */}                <button type="button" className="btn-ghost" title="Tag 'hide' on selected" onClick={() => applyTagToSubsSelection('hide')} disabled={subsSelectedVisibleSetDisplay.size === 0}>hide</button>                {/* Tags opener */}                <button type="button" className="btn-ghost" title="Open tagger" onClick={() => setShowTagger(v => !v)} disabled={subsSelectedVisibleSetDisplay.size === 0}>tags</button>                {/* Collections quick apply (Subs) */}                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>                  <select className="side-input" value={applyCollectionId} onChange={(e) => setApplyCollectionId(e.currentTarget.value)} title="Select collection">                    <option value="">(collection)</option>                    {collections.map(c => (<option key={c.id} value={c.id}>{c.name}</option>))}                  </select>                  <button type="button" className="btn-ghost" title="Add to collection" disabled={subsSelectedVisibleSetDisplay.size === 0 || !applyCollectionId} onClick={async () => {
        const ids = Array.from(subsSelectedVisibleSetDisplay);
    ...
    }}>                    -                  </button>                </span>                <span className="sel-info">{subsSelectedVisibleSetDisplay.size}{subsSelectedHiddenCount > 0 ? ` -${subsSelectedHiddenCount}` : ''}</span>              </div>)}              {mode === 'recommender' && (<div className="sel-controls" style={{ gap: 8 }}>                  <select className="side-input" value={recSetId} onChange={(e) => setRecSetId(e.currentTarget.value)} title="Rec Set">                    {recSets.map(s => (<option key={s.id} value={s.id}>{s.name}</option>))}                  </select>                  <button type="button" className="btn-ghost" title="Reshuffle" onClick={() => buildRecPage()}>Reshuffle</button>                  <label style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }} title="Exclude dontRecommend-tagged items">                    <input type="checkbox" checked={respectDontRecommend} onChange={(e) => setRespectDontRecommend(e.currentTarget.checked)} />                    Respect dontRecommend                  </label>                </div>)}              <span style={{ marginLeft: 'auto' }} />              {mode === 'subs' ? (<div style={{ display: 'inline-flex', alignItems: 'center', gap: 10 }}>                  <span className="muted" title="Most recent publish">Latest: {fmtRecent(subsMostRecentTs)}</span>                  <span className="muted" title="Last sub feed scrape">Scrape: {fmtTime(subsLastScrapeAt)}</span>                  <button type="button" className="btn-ghost" onClick={scrapeSubFeedNow}>Scrape</button>                </div>) : (<div style={{ display: 'inline-flex', alignItems: 'center', gap: 10 }}>                  {/* Recommender placeholder controls + collections quick apply */}                  <button type="button" className="btn-ghost" title="Shuffle (coming soon)">Shuffle</button>                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>                    <select className="side-input" value={applyCollectionId} onChange={(e) => setApplyCollectionId(e.currentTarget.value)} title="Select collection">                      <option value="">(collection)</option>                      {collections.map(c => (<option key={c.id} value={c.id}>{c.name}</option>))}                    </select>                    <button type="button" className="btn-ghost" title="Add to collection" disabled={selected.size === 0 || !applyCollectionId} onClick={async () => {
        ...
        if (!ids.length || !cid) return;
        await sendBg('videos/collections/apply', { ids, collectionId: cid, op: 'remove' } as any);
    }}>                      -                    </button>                  </span>                </div>)}            </div>          </header>          {/* Tagger (same UI reused) */}          {showTagger && subsSelectedVisibleSetDisplay.size > 0 && (<div className="tagger" style={{ padding: '8px 16px', borderBottom: '1px solid var(--border)', display: 'flex', gap: 12, flexWrap: 'wrap' }}>              {/* Video Type toggles (does not change tags) */}              <span style={{ marginRight: 8 }}>Apply tag:</span>              {(() => {                // Group tags by tag groups (same grouping as manager)                
